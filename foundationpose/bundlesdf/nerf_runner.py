...
class NerfRunner:
    ...
    def build_octree(self):
        ...
        #################### Dilate
        dilate_radius = int(
            np.round(self.cfg["octree_dilate_size"] / octree_smallest_voxel_size)
        )
    ...
    def mesh_texture_from_train_images(self, mesh, rgbs_raw, tex_res=1024):
        ...
        for i in range(len(rgbs_raw)):
            ...
            _, render_depth = renderer.render(
                mesh=mesh, ob_in_cvcam=np.linalg.inv(cvcam_in_ob)
            )
        ...
        mesh.visual = trimesh.visual.texture.TextureVisuals(
            uv=mesh.visual.uv, image=Image.fromarray(new_texture)
        )
        return mesh
